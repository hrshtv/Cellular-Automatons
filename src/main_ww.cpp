...
#include "Helpers.h"
#include "worlds/WireworldWorld.h"
...
class CustomWindow : public Fl_Window {
    ...
    WireworldWorld *world;

    public:
    CustomWindow(int w, int h, const char *title, WireworldWorld *world) : Fl_Window(w, h, title) {
        this->world = world;
    };
...
int main(){
    ...
    WireworldWorld w(R, C, CELL_SIZE);

    // Create the main window
    Fl_Window* window = new CustomWindow(R*CELL_SIZE, C*CELL_SIZE, "Wireworld", &w);
    window->color(FL_BLACK);

    Config *c = new Circuit();
    c->init();
